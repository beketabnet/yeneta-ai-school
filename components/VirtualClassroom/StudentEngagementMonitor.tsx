...
const StudentEngagementMonitor: React.FC<StudentEngagementMonitorProps> = ({ onEngagementChange }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 flex flex-col" style={{ height: '770px' }}>
      ...
      <div className="space-y-3 flex-1 flex flex-col">
        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5 flex-shrink-0">
          <div className={`h-2.5 rounded-full transition-all duration-500 ${config.barColor}`} style={{ width: config.width }}></div>
        </div>
        <div className={`flex items-center space-x-2 text-sm font-semibold ${config.color} flex-shrink-0`}>
          {config.icon}
          <span>{level} Engagement</span>
          <span className="text-xs text-gray-500 dark:text-gray-400 italic ml-auto">{config.text}</span>
        </div>

        <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2 space-y-1 border dark:border-gray-600 flex-1 flex flex-col">
          ...
          <div className="overflow-y-auto flex-1">
            {eventLog.length > 0 ? (
              eventLog.map(event => (
                <div key={event.timestamp} className="flex items-center space-x-2 text-xs text-gray-700 dark:text-gray-300 animate-fade-in py-1">
