...
import { Loader2, Lightbulb, CheckCircle, XCircle, FileQuestion, Activity, BrainCircuit, Settings } from 'lucide-react';
...
const Whiteboard: React.FC<WhiteboardProps> = ({ step, onQuizAnswer, isAdapting }) => {
  const { mode: imageGenerationMode } = useImageGeneration();
  ...
  useEffect(() => {
    if (step.type === LessonStepType.IMAGE) {
      const fetchImage = async () => {
        ...
        setImageUrl(null);
        try {
          const url = await generateImage(step.content, imageGenerationMode);
          setImageUrl(url);
        } catch (err: any) {
  ...
  const getModeDisplayName = (mode: ImageGenerationMode) => {
    switch (mode) {
      case ImageGenerationMode.GOOGLE_AI_BILLING:
        return "Google AI";
      case ImageGenerationMode.TEXT_ONLY:
        return "Text Only";
      case ImageGenerationMode.ALTERNATIVE_APIS:
        return "Free APIs";
      default:
        return mode;
    }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 h-[524px] relative flex flex-col">
      <div className="flex-shrink-0 flex items-center justify-between mb-4 pb-4 border-b dark:border-gray-700">
        ...
        <div className="flex items-center space-x-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          <span>{getModeDisplayName(imageGenerationMode)}</span>
        </div>
      </div>
