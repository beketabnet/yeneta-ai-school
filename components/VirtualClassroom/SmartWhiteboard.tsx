...
import { MousePointer2, Pencil, Eraser, Type, Trash2, Undo2, Redo2, ImagePlus, Video, Circle, RectangleHorizontal, Minus, Play, Pause, StopCircle, AlertTriangle, Globe } from 'lucide-react';
...
const SmartWhiteboard = forwardRef((props, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        getCanvasState: async (): Promise<{ json: any; image: string }> => {
            ...
            tempCanvasEl.width = canvas.getWidth();
            tempCanvasEl.height = canvas.getHeight();
            const tempFabricCanvas = new fabric.StaticCanvas(tempCanvasEl, {
                backgroundColor: canvas.backgroundColor,
            });

            const objectPromises = canvas.getObjects().map((obj: any) => {
                return new Promise<void>(resolve => {
                    ...
                    if (isVideo) {
                        ...
                        frameCanvas.width = videoEl.videoWidth || 640; // provide fallbacks
                        frameCanvas.height = videoEl.videoHeight || 360;
                        const ctx = frameCanvas.getContext('2d');
                        if (ctx) {
                            ctx.drawImage(videoEl, 0, 0, frameCanvas.width, frameCanvas.height);
                            const frameImage = new fabric.Image(frameCanvas);
                            frameImage.set({
                            ...
                            tempFabricCanvas.add(frameImage);
                        }
                        resolve();
                    } else {
            ...
            const image = tempFabricCanvas.toDataURL({ format: 'png' }).split(',')[1];
        ...
        aiAddImage: (imageUrl: string, options: any) => {
            ...
            fabric.Image.fromURL(imageUrl, (fabricImage) => {
                if (isUnmounting.current || !fabricImage) {
                    console.error("AI failed to load generated image using fabric.Image.fromURL.");
                    return;
                }
                fabricImage.set({
        ...
        aiAddVideo: (videoUrl: string, options: any) => {
            const canvas = fabricRef.current; if (!canvas) return;
            const onAdd = (obj: any) => {
                obj.set({ left: options.left || 0, top: options.top || 0 });
                obj.scaleToWidth(canvas.getWidth() * 0.5);
            ...
            videoEl.addEventListener('canplay', () => {
                if (isUnmounting.current) { URL.revokeObjectURL(videoUrl); return; }
                videoEl.width = videoEl.videoWidth;
                videoEl.height = videoEl.videoHeight;
                const fabricVideo = new fabric.Image(videoEl, { objectCaching: false });
                onAdd(fabricVideo);
                videoEl.addEventListener('play', manageRenderLoop);
                videoEl.addEventListener('pause', manageRenderLoop);
                videoEl.addEventListener('ended', manageRenderLoop);
            }, { once: true });

            videoEl.onerror = () => { console.error("Error loading AI-generated video."); URL.revokeObjectURL(videoUrl); };
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (observer) { observer.disconnect(); resizeObserverRef.current = null; }
            if (drawingTimeout) clearTimeout(drawingTimeout);
            ...
            if (canvasToDispose) {
                ...
                } catch (e) { console.error("Error during robust Fabric.js canvas cleanup:", e); }
            }
            fabricRef.current = null;
        };
    }, []);

    // --- Cleanup for Confirmation Timeout ---
    useEffect(() => {
        return () => {
        ...
        if (fileType === 'image') {
            ...
            reader.onload = (f) => {
                ...
                fabric.Image.fromURL(dataUrl, (fabricImage) => {
                    if (isUnmounting.current || !fabricImage) {
                        console.error("Fabric.js failed to load image from data URL.");
                        return;
                    }
                    onAdd(fabricImage);
                }, { crossOrigin: 'anonymous' });
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-4 flex flex-col" style={{ height: '650px' }}>
            <div className="flex-shrink-0 space-y-3">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2 bg-white dark:bg-gray-800 rounded-lg px-3 py-2 shadow-md border border-gray-200 dark:border-gray-700">
                ...
                <div className="flex flex-wrap items-center gap-2 p-2 rounded-lg bg-gray-100 dark:bg-gray-900 border dark:border-gray-700">
                    ...
                    <button
                        ...
                        className={`p-2 rounded-md transition-colors ${isConfirmingClear
                            ? 'bg-red-500 text-white animate-pulse'
                            : 'bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400 hover:bg-red-200'}`}
                        title={isConfirmingClear ? "Confirm Clear" : "Clear Canvas"}
                    >
