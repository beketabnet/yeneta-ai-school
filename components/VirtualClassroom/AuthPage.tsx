...
import { loginUser, registerUser, type UserInfo } from '../services/apiService';
import { Loader2, UserPlus, LogIn, Eye, EyeOff, ArrowLeft, BookOpen } from 'lucide-react';
...
const AuthPage: React.FC<AuthPageProps> = ({ onLoginSuccess, onGuestLogin, onBackToLanding }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isLoginView) {
                ...
                await registerUser({ username, email, password, password_confirm: confirmPassword, role });
        ...
        <div className="max-w-md mx-auto mt-10 bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            {onBackToLanding && (
                <button
                    ...
                    className="flex items-center space-x-2 text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors duration-200 mb-6 group"
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {!isLoginView && (
                    <div>
                        ...
                        <input type="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="mt-1 block w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900 dark:text-gray-200" />
                    </div>
                )}
                ...
                {!isLoginView && (
                    <div>
                        ...
                        <select 
                            ...
                            required 
                            className="mt-1 block w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900 dark:text-gray-200"
                        >
                            <option value="student">Student - Learn with AI-powered lessons</option>
                            <option value="guide">Teacher/Guide - Create and manage lessons</option>
                            <option value="staff">Staff - Assist with classroom management</option>
