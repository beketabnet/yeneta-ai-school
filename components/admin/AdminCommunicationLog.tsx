/**
 * AdminCommunicationLog Component
 * Professional communication interface for Administrator Dashboard
 * Allows admin to message with Teachers and Parents
 * Features: Rich media attachments, file uploads, audio/video recording
 */
...
const MessageStatus: React.FC<{ status: 'sent' | 'delivered' | 'read' | undefined }> = ({ status }) => {
    ...
    const iconClass = isRead ? 'text-blue-500 w-3 h-3' : 'text-gray-400 dark:text-gray-500 w-3 h-3';

    // Just for visual polish, let's wrap it slightly
    return (
...
const AttachmentPreview: React.FC<{ attachment: File, onRemove: () => void }> = ({ attachment, onRemove }) => {
    return (
        <div className="relative group inline-flex items-center gap-2 p-2 pr-8 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl text-sm mb-3 animate-in fade-in slide-in-from-bottom-2">
...
const AdminCommunicationLog: React.FC = () => {
    ...
    useEffect(() => {
        const fetchConversations = async () => {
            ...
            try {
                ...
                // Select first conversation only if none selected and not on mobile (conceptually)
                // For now, auto-selection is fine
            ...
            <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-3xl border border-gray-200 dark:border-gray-700 shadow-xl overflow-hidden flex flex-col h-[85vh] max-h-[900px]">
                ...
                <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-white/50 dark:bg-gray-800/50">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">Secure admin communication channel</p>
                    </div>
                    {error && (
                            ...
                            <div className="flex items-center gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-9 pr-3 py-2 bg-white dark:bg-gray-800 border-none rounded-xl text-sm shadow-sm ring-1 ring-gray-200 dark:ring-gray-700 focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                                    />
                                </div>
                                <button
                                    onClick={() => setIsNewConversationModalOpen(true)}
                                    className="p-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl shadow-lg shadow-blue-500/30 hover:shadow-blue-500/40 active:scale-95 transition-all"
                        ...
                        <div className="flex-1 overflow-y-auto px-2 custom-scrollbar">
                            {isLoadingConvos ? (
                                ...
                                <div className="text-center py-10 px-4">
                                    ...
                                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Start a new chat to connect with users.</p>
                                </div>
                            ) : (
                                <div className="space-y-1 pb-4">
                                    {filteredConversations.map(convo => {
                                        ...
                                        const avatarBg = otherUser?.role === 'Teacher' ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' :
                                            otherUser?.role === 'Student' ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' :
                    ...
                    <div className="flex-1 flex flex-col bg-white/30 dark:bg-gray-800/30 backdrop-blur-sm relative">
                        {!selectedConversation ? (
                            <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-8 text-center">
                                ...
                                <p className="text-sm max-w-xs mt-2">Select a conversation from the sidebar to start chatting with teachers, students, or parents.</p>
                            </div>
                        ) : (
                            <>
                                {/* Chat Header */}
                                    ...
                                    <form onSubmit={handleSendMessage} className="flex gap-2 items-end">
                                        <div className="flex gap-1 mb-1">
                                            ...
                                            <button
                                                ...
                                                disabled={isSending}
                                                className="p-2.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-colors"
                                                title="Attach File"
                                            >
                                        ...
                                        <div className="flex-1 relative">
                                            <textarea
                                                ...
                                                rows={1}
                                                className="w-full pl-4 pr-12 py-3 bg-gray-50 dark:bg-gray-900 border-none rounded-2xl resize-none shadow-inner text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all custom-scrollbar max-h-32"
                                                disabled={isSending}
                                            ...
                                            <button
                                                ...
                                                className="absolute right-2 bottom-2 p-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white rounded-xl shadow-md transition-all active:scale-95"
                                            >
                                                {isSending ? (
...
export default AdminCommunicationLog;
