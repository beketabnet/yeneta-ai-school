...
class Command(BaseCommand):
    help = 'Setup comprehensive test data for gradebook tests'
    ...
    def handle(self, *args, **options):
        try:
            ...
            for i, (first_name, last_name) in enumerate(student_names, 1):
                ...
                self.stdout.write(f"✓ Created/Found student: {first_name} {last_name}")
            ...
            for subject in subjects[:3]:  # Only first 3 subjects
                for grade in grades[:6]:  # Only first 6 grades
                    ...
                    if not created and course_req.status != 'approved':
                        course_req.status = 'approved'
                        course_req.reviewed_by = admin
                        course_req.reviewed_at = timezone.now()
                        course_req.save()
                    if created:
                        self.stdout.write(f"✓ Created approved course: {subject} - Grade {grade}")
            ...
            for i, student in enumerate(students):
                ...
                for subject in subjects[:3]:
                    ...
                    if not created and enrollment.status != 'approved':
                        ...
                        enrollment.save()
                    if created:
                        self.stdout.write(f"✓ Created enrollment: {student.first_name} -> {subject} - {student_grade}")
            ...
            for student in students:
                for subject in subjects[:3]:
                    ...
                    for assignment_type in assignment_types:
                        ...
                        if created:
                            grade_count += 1
                            self.stdout.write(f"✓ Created grade: {student.first_name} - {subject} - {assignment_type}: {score}")
                    
                    # Create exam grades (for half the students)
                    if hash(f"{student.id}_{subject}") % 2 == 0:
                        for exam_type in exam_types:
                            ...
                            if created:
                                grade_count += 1
                                self.stdout.write(f"✓ Created exam grade: {student.first_name} - {subject} - {exam_type}: {score}")
            
            self.stdout.write(self.style.SUCCESS("\n✓ Test data setup complete!"))
            self.stdout.write(f"Created {len(students)} students")
            self.stdout.write(f"Created {len(approved_courses)} approved courses")
            self.stdout.write(f"Created {grade_count} grades")
            self.stdout.write("Students are enrolled in these courses and have grades entered")
